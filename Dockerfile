...
RUN cd /source && mkdir -p .conan && cd .conan && conan profile detect && conan install .. -s build_type=Debug -s compiler.libcxx=libstdc++11 -s compiler.cppstd=20 -o "boost/*:without_cobalt=True" --build missing
