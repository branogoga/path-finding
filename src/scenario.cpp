...
JobRequest FileScenarioLoader::parseJobRequest(const std::string& line, const std::string& filename) {
    ...
    if(!graphLoader) {
        ...
        const std::string graphFilename = std::filesystem::path(filename).replace_filename(graphFilenamePath.filename()).replace_extension(graphFilenamePath.extension()).string();
    ...
    if(!startVertex) {
        std::ostringstream message;
        message << "Failed to open '" << filename << "': invalid format. Unable to convert map position '" << startX << "', '" << startY << "' to vertex index.";
        throw std::invalid_argument(message.str());
    }
